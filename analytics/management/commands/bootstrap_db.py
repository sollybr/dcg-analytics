...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not table_exists:
                self.stdout.write(self.style.WARNING(f"Table '{table_name}' is missing. Cleaning up stale migration history..."))
                with connection.cursor() as cursor:
                    cursor.execute("DELETE FROM django_migrations WHERE app = 'analytics';")
                self.stdout.write(self.style.SUCCESS("Stale migration history cleared successfully."))
        except Exception:
        ...
        try:
            sync_cards()
            self.stdout.write(self.style.SUCCESS("Cards synchronized and database populated successfully!"))
